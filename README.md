...
constructor(container: HTMLElement, events: IEvents) - Инициализирует элементы страницы и назначает обработчики событий.Методы:
...
Методы
getProductList(): Promise<IProduct[]> : Получение списка всех продуктов с сервера. Преобразует пути изображений, используя CDN.
getProductItem(id: string): Promise<IProduct> : Получение детальной информации о продукте по его ID. Преобразует путь изображения, используя CDN.
